...
void ace_auto(const char *operation){
	...
	if (strcmp(operation, "fill") == 0){
		...
		ace_fake_td_ams( ACE_Element[23], ACE_Isotope[23] ); 
...
void ace_fake_td_ams(const char *element, Particle::Type isotope){
	...
	TFile *file0;
	if (!strcmp(element, "B") || !strcmp(element, "C") || !strcmp(element, "N") || !strcmp(element, "O")) file0 = new TFile(Form("data/ACE/compare/fit_%s_%dnodes.root", element, nnodes)); 
	else file0 = new TFile(Form("data/ACE/compare/fit_%s_%dnodes.root", ACE_Element[choose_BO(element)], nnodes)); 
...
void ace_fake_td_ams_v4(const char *element, Particle::Type isotope){
	...
	for (int iBR=0; iBR<nBRs; ++iBR){
		...
		if (!strcmp(element, "B")){ 
			...
			fit_ratio->SetParError(0, g_correrr[0]->GetEY()[h_correrr[0]->FindBin(h_ace_BR->GetBinContent(best_bin[0]+2))-1] );
...
*/
	...
	for (int i=0; i<1; ++i){
		...
		for (int k=0; k<1; ++k){
			...
			g_chi2_ams->GetXaxis()->SetTimeOffset(0,"1970-01-01 00:00:00"); 
			g_chi2_ams->GetXaxis()->SetTitleSize(0.7);
			g_chi2_ams->GetYaxis()->SetRangeUser(0., 10.); 
			g_chi2_ams->Draw("APL"); 
			l_c5_2->Draw("SAME"); 			
