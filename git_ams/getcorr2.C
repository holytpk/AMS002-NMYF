//Get correlation and correlation coefficients for selected neutron monitor stations
...
void getcorr2(){
   ...
   const char *NM_useful[nNMs_useful] = { "NM-PSNM", "NM-MXCO", "NM-HRMS", "NM-JUNG", "NM-JUNG1", "NM-NEWK", "NM-KIEL2", "NM-OULU", "NM-APTY", "NM-FSMT", "NM-NAIN", "NM-PWNK", "NM-THUL", "NM-SOPB", "NM-SOPO" };
   ...
   for(int i=0; i<=nNMs_useful-1; i++){
   	...
   	for(int j=i+1; j<=nNMs_useful-1; j++){
		...
		c1->cd(1);
		//read in first file
		TFile fin1(Form("./data/nm/%s.root", NM_useful[i]));
		...
		if(g1_ave==NULL){
			...
			g1_ave->SetTitle(Form("%s Flux & %s Flux;Time;Flux [1/(m^2 sr s GV)]", NM_useful[i], NM_useful[j]));
		...
		//read in second file
		TFile fin2(Form("./data/nm/%s.root", NM_useful[j]));
		...
		if(g2_ave==NULL){
			...
			g2_ave->SetTitle(Form("%s Flux & %s Flux;Time;Flux [1/(m^2 sr s GV)]", NM_useful[i], NM_useful[j]));
		...
		gcor->SetTitle(Form("Correlation;%s Count Rate [Hz];%s Count Rate [Hz]", NM_useful[i], NM_useful[j]));
		//cor->Print();
		gcor->Draw("AP");
		...
		double corr_low=0, corr_up=0; // these two variables will contain the lower and upper value of the correlation, accounting for statistical uncertainty at a given confidence level defined below
	
		double cl = TMath::Erf(1/sqrt(2)); // confidence level corresponding to 1 sigma, roughly 0.68, which means ~68% probability

		double corr_pvalue = HistTools::CorrelationZTest(corr, gcor->GetN(), corr_low, corr_up, cl); // the p-value is a number that tells you how much is probable that the correlation factor you computed is different from zero simply by chance, and not by some physical reason
		...
		//c1->SaveAs(Form("./data/corr/%s_%s.png", NM_useful[i], NM_useful[j]));
   	}

   } //end of nNMs_useful for loop 
